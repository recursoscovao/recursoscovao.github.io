...
// Estilos específicos para este jogo (Capa e Destaque)
...
gameStyle.innerHTML = `
    ...
    /* Faz o animal ficar pequeno e nunca tocar nas bordas */
    .destaque-box img {
    ...
    /* Botões da Capa Controlados pelo Jogo */
    .btn-jogo-audio { width: 65px; height: 65px; cursor: pointer; transition: 0.2s; }
    .btn-jogo-audio:active { transform: scale(0.9); }
    .btn-jogo-play { flex: 1; height: 65px; border-radius: 35px; background: var(--primary-color); color: white; border: none; font-size: 1.5rem; font-weight: 900; cursor: pointer; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
`;
document.head.appendChild(gameStyle);

// ==========================================
// LOGICA DA CAPA E ÁUDIO
// ==========================================
function mostrarCapa() {
    if (jogoAtivo) return;
    
    // 1. Título do Jogo no Header
    ...
    // 3. Botões no Footer (Controlados aqui!)
...
function tocarAudioInstrucoes() {
    ...
    audioInstrucoes.play().catch(() => console.warn("Ficheiro instrucoes.mp3 não encontrado."));
}
...
function correrSimulacao() {
    ...
    const animar = () => {
        ...
        const imgDestaque = document.getElementById('simu-destaque');
        if(!imgDestaque) return;

        imgDestaque.src = DADOS_JOGO.caminhoImagens + itens[certoIdx].img;
        itens.forEach((it, i) => { 
        ...
        hand.style.display = "block"; hand.style.opacity = "0"; hand.style.transition = "none";
        hand.style.top = "80%"; hand.style.left = "80%";
...
function verificarResposta(id, el) {
    ...
    if (id === itemDestaque.id) {
        ...
        el.style.borderColor = "#8cc63f"; el.style.background = "#f0fff0";
    } else {
        erros++; somErro.play();
        el.style.borderColor = "#ff5a5f"; el.style.background = "#fff5f5";
