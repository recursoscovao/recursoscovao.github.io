...
style.innerHTML = `
    ...
    /* 2.1 PC (ECRÃS GRANDES) */
    @media screen and (min-width: 1025px) { 
        :root { --cell-size: min(55px, 7.5vh); } 
    }

    /* 2.2 TABLET VERTICAL (iPad) */
    ...
    /* 2.4 MODO HORIZONTAL (LANDSCAPE) - TODAS AS TELAS */
...
function mostrarCapa() {
    ...
    // Preparação de Instruções e Feedback
        ...
        panel.innerHTML = `
            ...
            <div class="inst-content">
                ...
                <ul class="inst-list">
                    ...
                    <li><b>Diagonal:</b> Move para vazia ou para capturar peça adversária.</li>
                </ul>
            </div>`;
...
function atualizarUI() {
    ...
    const nomeVez = (turnoAtual === 0) ? "J1" : pcLabel;
    const classPill = (turnoAtual === 0) ? "pill-j1" : "pill-j2";
    const modoTexto = modoJogo === 'CPU' ? "J1 vs Pc" : "J1 vs J2";
    
    // BARRA DE STATUS PADRONIZADA
    document.getElementById('shell-header-content').innerHTML = `
        <div class="status-container" style="width:100%; display:flex; justify-content:space-between; align-items:center;">
            <div style="display:flex; flex-direction:column; align-items:flex-start;">
                ...
                <div style="font-size:0.65rem; font-weight:800; color:#aaa; margin-top:2px; margin-left:5px;">${modoTexto}</div>
            </div>
...
function finalizarRonda(vencedorIdx) {
    ...
    overlay.innerHTML = `<div class="vitoria-card"><h1 style="color:#8cc63f; font-size:2rem; font-weight:900;">${nomeV}</h1><p>Ganhou a ronda!</p><div style="margin-top:10px; font-weight:800;">PLACAR: J1 ${matchScore[0]} - ${matchScore[1]} ${pcLabel}</div></div>`;
